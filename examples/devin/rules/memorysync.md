...
You have MemorySync long-term memory (the `memorysync` MCP tools). Your conversation here is captured automatically when the MemorySync hooks are installed; recall is yours to drive:
...
- When a durable fact appears (a preference, correction, decision, convention), call `add_memory` with ONE clear self-contained statement. Never store secrets, tokens or passwords.
